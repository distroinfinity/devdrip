...
export async function saveAlerts(replacement: AlertReplacement[]): Promise<SaveAlertsResult> {
  try {
    const data = await apiFetchOrRefresh<{ alerts: AlertDto[] }>(
      "/me/alerts",
      "/dashboard/preferences",
      {
        method: "PUT",
        body: JSON.stringify({ alerts: replacement }),
        headers: { "Content-Type": "application/json" },
      }
    )

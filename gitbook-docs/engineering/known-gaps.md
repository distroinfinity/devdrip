...
- admin surface uses X-Admin-Secret (shared secret, no caller identity or audit trail) — should converge on authenticated admin principals for production. Admin routes now include `/admin/stats`, `/admin/users`, `/admin/payouts`, `/invites` in addition to the existing advertiser/campaign/creative CRUD.

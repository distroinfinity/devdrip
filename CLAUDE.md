...
- **never publish the split or exact rates.** The revenue structure in public copy is: _advertisers pay Distro, Distro pays the developer a share, and keeps a margin_. No "70%", no CPM figures, on the site, dashboard or CLI. The developer angle: idle agent time earns money and helps pay for the AI tools that cause it. (`REVENUE_SHARE_DEVELOPER` and `AD_CPM_RATE` stay internal.)

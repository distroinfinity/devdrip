...
import { EarningsStrip } from "@/components/dashboard/overview/earnings-strip"
import type { EarningsSummary, ReadingListResponse } from "@/lib/dashboard-api"
...
export default async function DashboardOverview() {
  ...
  const [
    ...
    // an earnings failure renders no strip — it never breaks the overview
    apiFetchOrRefresh<EarningsSummary>("/me/earnings/summary", "/dashboard").catch(() => null),
  ])

  return (
    <>
      {earnings && <EarningsStrip summary={earnings} />}
      <OverviewClient
        session={session}
        sessionToken={token}
